<template>
    <Modal title="拥有权限" width="700" v-model="visible" @on-cancel="hide">
        <div class="user-privs" v-loading="loading">
        ...
        async getUserPrivs() {
            try {
                ...
                if (data.success) {
                    ...
                    const allPrivsTreeData = parsePrivsToTreeDataWithChecked(this.allPrivs, privs, 0);
                    const adminRouteIndex = allPrivsTreeData.findIndex(item => item.privCode === 'system');
                    this.allPrivsTreeData = allPrivsTreeData.filter((_, index) => index !== adminRouteIndex); // 取消综合管理的权限分配
                    const selectedIds = privs.map(item => item && item.privId).filter(item => !!item);
